...
async def match_grant_from_post(post_text: str) -> Optional[dict[str, Any]]:
    """Given the text of a channel post, find the grant it's about.

    Our posts lead with the grant title, so the first line is the strongest
    signal; we fall back to searching the whole post text.
    """
    if not post_text:
        return None
    first_line = post_text.strip().splitlines()[0].strip() if post_text.strip() else ""
    for query in (first_line, post_text):
        if not query:
            continue
        res = await search_grants(query, limit=1)
        if res:
            return res[0]
    return None


def format_grant_brief(grant: dict[str, Any], lang: str = "en") -> str:
    """A short, grounded one/two-liner (used as a no-LLM fallback under comments)."""
    e = html.escape
    parts = [f"🎓 <b>{e(_localize(grant, 'title', lang))}</b>"]
    dl = _fmt_deadline(grant.get("deadline", ""), lang)
    if dl:
        parts.append(f"{_lbl('deadline', lang)} {dl}")
    url = (grant.get("url") or "").strip()
    if url:
        parts.append(f'{_lbl("register", lang)} <a href="{e(url)}">{_lbl("link_word", lang)}</a>')
    return "\n".join(parts)
